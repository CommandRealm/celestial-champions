...
##Fail safe for if the owner player is still too close
execute if entity @a[scores={calculate=0},distance=..5,gamemode=adventure,tag=is_fighter] run tag @s remove about_to_explode

execute if entity @s[tag=about_to_explode] run function fighter:incend/tnt_explode
