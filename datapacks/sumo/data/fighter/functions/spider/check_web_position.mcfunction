...
execute if entity @a[scores={calculate=1..},distance=..5,gamemode=adventure,tag=is_fighter] run tag @s add about_to_activate
execute if entity @a[scores={calculate=..-1},distance=..5,gamemode=adventure,tag=is_fighter] run tag @s add about_to_activate

execute if entity @s[tag=about_to_activate] run function fighter:entity_death_timer_over
##Remove temp tag
