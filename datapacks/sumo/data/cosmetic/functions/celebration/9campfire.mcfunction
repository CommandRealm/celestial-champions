...
execute as @a[tag=winner,scores={celebration=9}] at @s if score $end cooldown matches 99 run give @s stick{CustomModelData:1,HideFlags:1,display:{Name:'[{"translate":"Stick","color":"gold","italic":false}]'}}
