...
execute if score $players_in_game calculate matches ..1 run function game:not_enough_players
...
execute if score $time game matches 109.. unless score $end cooldown matches 0.. unless score $time cooldown matches 1.. as @a[tag=playing,gamemode=adventure] at @s unless entity @e[type=area_effect_cloud,tag=map_marker,distance=..250] run function game:left_map

##if there is a spectator
execute if entity @a[team=spectator] run function game:spectator/main

##Spectator limits
execute as @a[gamemode=spectator,x=0,y=66,z=0,distance=500..] at @s if entity @a[gamemode=adventure,tag=playing] unless entity @a[tag=playing,gamemode=adventure,distance=..100] at @p[tag=playing,gamemode=adventure] facing entity @s eyes run tp @s ^ ^ ^30 facing entity @s
