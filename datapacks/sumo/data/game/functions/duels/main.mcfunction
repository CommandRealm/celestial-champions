...
##Tps spectators to their proper position.
execute if score $time game matches 8 as @e[type=area_effect_cloud,tag=map_marker] at @s run kill @e[tag=on_death_timer,distance=..100,tag=!map_marker]
...
execute as @a[tag=playing,gamemode=adventure] at @s unless entity @e[type=area_effect_cloud,distance=..300,scores={death_timer=1..}] run scoreboard players remove @s duels_timer 1
