...
execute store result score $gray distribution if entity @a[tag=playing,tag=on_white_team]
...
scoreboard players operation $check distribution < $gray distribution
...
scoreboard players operation $gray distribution -= $check distribution
...
execute if score $gray distribution matches 0 as @r[tag=playing,tag=!on_team] at @s run function game:team_distribution/join_gray
execute if entity @a[tag=!on_team,tag=playing] run function game:team_distribution/ten_teams