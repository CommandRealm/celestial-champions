...
setblock 7 64 24 oak_wall_sign[facing=west]{Text1:'{"text":"","color":"blue","bold":true,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 1.. run function options:error"}}',Text2:'{"text":"Lives:","color":"aqua","bold":true}',Text3:'{"score":{"objective":"settings","name":"$lives"},"color":"gold"}',Text4:'{"text":"<<<         >>>","color":"#d4d4d4","bold":true,"clickEvent":{"action":"run_command","value":"/execute as @s at @s anchored eyes if score $game state matches 0 run function options:lives/click_life"}}'}
