...
setblock 7 64 24 oak_wall_sign[facing=west]{front_text:{messages:['{"translate":"","color":"blue","bold":true,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 1.. run function options:error"}}', '{"translate":"Kills to win:","color":"aqua","bold":true}', '{"score":{"objective":"settings","name":"$kills_to_win"},"color":"gold"}', '{"translate":"<<<         >>>","color":"#d4d4d4","bold":true,"clickEvent":{"action":"run_command","value":"/execute as @s at @s anchored eyes if score $game state matches 0 run function options:kills_to_win/click_kill"}}'], has_glowing_text: false, color: "black"}}
