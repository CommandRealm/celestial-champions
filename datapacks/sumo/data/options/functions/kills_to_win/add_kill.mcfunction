...
setblock 7 64 24 oak_wall_sign[facing=west]{Text1:'{"text":"","color":"blue","bold":true,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 1.. run function options:error"}}',Text2:'{"text":"Kills to win:","color":"aqua","bold":true}',Text3:'{"score":{"objective":"settings","name":"$kills_to_win"},"color":"gold"}',Text4:'{"text":"<<<         >>>","color":"#d4d4d4","bold":true,"clickEvent":{"action":"run_command","value":"/execute as @s at @s anchored eyes if score $game state matches 0 run function options:kills_to_win/click_kill"}}'}
