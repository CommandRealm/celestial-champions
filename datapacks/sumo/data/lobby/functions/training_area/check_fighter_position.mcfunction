...
execute unless entity @e[type=falling_block,distance=..3,tag=gracea_block] unless entity @e[type=falling_block,distance=..3,tag=deployable_platform] unless entity @s[scores={ability_type=1,ability_state=1,fighter=13}] run function lobby:fall_off
