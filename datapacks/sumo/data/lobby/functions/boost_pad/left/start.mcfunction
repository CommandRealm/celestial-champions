...
##Generating actual id
scoreboard players operation @s id = @s lobby_id
scoreboard players add @s id 51
...
execute as @e[type=armor_stand,tag=needs_adjustments,sort=nearest,limit=1] at @s run data merge entity @s {Motion:[3.15d,2.05d,0.0d],OnGround:0b}
