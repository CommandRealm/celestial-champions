...
execute if score $players_in_game calculate matches ..49 unless score $mode settings matches 3..4 as @a[tag=!playing,gamemode=adventure,tag=!is_fighter] at @s unless entity @s[nbt={Inventory:[{Slot:3b,id:"minecraft:knowledge_book",tag:{book_type:"join"}}]}] run function lobby:midgame/get_join_book
